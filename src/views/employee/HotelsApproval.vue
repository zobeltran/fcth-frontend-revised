<template>
  <v-container grid-list-md>
    <v-layout row wrap>
      <v-flex xs12 sm6 md4 lg4 v-for="hotel_alias in viewHotelsForApproval" :key="hotel_alias">
        <v-container>
          <v-card>
            ...
            <v-card-actions>
              <v-btn
                ...
                @click="approveHotels(hotel_alias.id)"
              >
...
export default {
  ...
  methods: {
    fetchData() {
      this.$store
        .dispatch("viewHotelsForApproval")
        .then(response => {
          this.viewHotelsForApproval = response.data.hotels;
        })
        .catch(response => {
          if (error.status === 400) {
    ...
    approveHotels(id) {
      this.$store
        .dispatch("approveHotels", { id: id, isApproved: true })
        .then(response => {
          this.fetchData();
          if (response.status === 200) {
            this.snackbar.color = "success";
            this.snackbar.isError = true;
            this.snackbar.text = "Hotel has been approve. Refreshing list in 3 seconds";
          }
        })
        .catch(error => {
          if (error.status === 400) {
