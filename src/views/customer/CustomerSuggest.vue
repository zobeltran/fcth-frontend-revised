    ...
    <v-layout row wrap>
        <v-container>
          ...
          <v-form ref="form" v-model="valid">                                    
            ...
            <v-btn flat color="blue" @click.exact="suggest">SUGGEST</v-btn>
        </v-card-actions>
      </v-card>
    </v-container>
    <v-snackbar
    ...
    methods: { 
        suggest() {
            if (this.$refs.form.validate()) {
                this.load = true;
                this.$store.dispatch('suggestPackage', {
                    email: this.email,
                    id: this.$route.query.id
                })
                .then((response) => {
                    console.log( "INSIDE SUCCESSFUL CALLBACK" )
                ...
                .catch((error) => {
                	console.log( 'INSIDE EXCEPTION CALLBACK' )
                    if (error.status === 500) {
                        this.load = false;
                        this.snackbar.isError = true;
          ...
          this.$store
            ...
            .then(response => {
                this.package_alias = response.data.packages
            })
            .catch(response => {
              if (error.status === 400) {
                this.snackbar.isError = true;
