      ...
      <v-card v-for="bookings in viewticketsBookings" :key="bookings">
        ...
        <v-card-actions>
          <vue-stripe-checkout
            ...
            :name="bookings.ticket.flightNo + ' - ' + bookings.referenceNumber"
            currency="PHP"
            :amount="bookings.ticket.price * 100"
            @done="done"
            @opened="opened"
            @closed="closed"
            @canceled="canceled"
          ...
          <v-btn
            ...
            flat
            color="blue"
            @click.exact="checkout(bookings.referenceNumber, bookings.ticket.price * 100, bookings.ticket.flightNo + ' - ' + bookings.referenceNumber)"
          ...
          <v-btn
            ...
            flat
            color="red"
            @click="cancelReservation(ticket_alias.id)"
          >Cancel</v-btn>
          <v-btn v-else flat color="blue" disabled>PAID</v-btn>
          <v-btn
            v-if="!bookings.isPaid"
...
export default {
  ...
  methods: {
    book(id) {
      this.$store
        .dispatch("ticketsBookings", { ticket: id })
        .then(response => {
          this.fetchData();
